...
# So Basically I am making a complete API for ElGamal Encryption and Decryption
def get_args():
...
def encrypt(PubKey, msg, debug=False):
	p, alfa, B = PubKey
	a = randint(0, p) # = 4
	...
	sent_values = (Ke, y)
	if debug:
		print ("Plaintext message:       "  + str(msg) )
...
def decrypt(privateKey, E_msg, debug=False):
	p, alfa, b = privateKey
	Ke, y = E_msg
...
def sign(PrivateKey, msg, debug=False):
	p, alfa, b = PrivateKey
...
def create_PrivateKey(p, alfa, b):
	p =      int(p)
	alfa =   int(alfa)
	b =      int(b)
...
def main():
	...
	if myargs.encrypt:
		...
		if (p is None) or (alfa is None) or (B is None):
			encrypt_help()
		Kpub = create_PublicKey(p, alfa, B)
		...
		if (p is None) or (alfa is None) or (b is None) or (Ke is None):
			decrypt_help()	
		Kpriv = create_PrivateKey(p, alfa, b)
		Ke =    int(Ke)
